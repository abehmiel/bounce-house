...
**Goal:** Add a `bounce-house explain` subcommand that provides tiered metric documentation with fuzzy matching and a `--technical` flag.

**Architecture:** New `metric_docs.py` module holds all metric documentation as dataclasses. CLI gains an `explain` subparser. Report module gains explain-specific formatters. Three levels: overview (all), module, single metric. Fuzzy matching resolves partial names. `--technical` appends measurement details.

**Tech Stack:** Python dataclasses, existing ANSI formatting from `report.py`, no new dependencies.

---

### Task 1: MetricDoc Data Model and Lookup Function

**Files:**
- Create: `src/bounce_house/metric_docs.py`
- Create: `tests/test_explain.py`

**Step 1: Write failing tests for data model and lookup**
...
Expected: FAIL — `ModuleNotFoundError: No module named 'bounce_house.metric_docs'`
...
```bash
git add src/bounce_house/metric_docs.py tests/test_explain.py
git commit -m "feat: add MetricDoc data model with 21 metrics and fuzzy lookup"
```

---

### Task 2: Explain Formatters in report.py

**Files:**
- Modify: `src/bounce_house/report.py`
- Modify: `tests/test_explain.py` (append new tests)

**Step 1: Write failing tests for explain formatting**
...
Run: `uv run pytest tests/test_explain.py::TestFormatExplainOverview -v`
Expected: FAIL — `ImportError: cannot import name 'format_explain_overview' from 'bounce_house.report'`

**Step 3: Write implementation**

Add to `src/bounce_house/report.py` (after existing functions, before `_format_duration`):
...
```bash
git add src/bounce_house/report.py tests/test_explain.py
git commit -m "feat: add explain formatting functions to report module"
```

---

### Task 3: Wire explain Subcommand into CLI

**Files:**
- Modify: `src/bounce_house/cli.py`
- Modify: `tests/test_explain.py` (append CLI tests)

**Step 1: Write failing tests for CLI wiring**
...
Run: `uv run pytest tests/test_explain.py::TestExplainCLI -v`
Expected: FAIL — argparse doesn't know about `explain` subcommand

**Step 3: Write implementation**

Modify `src/bounce_house/cli.py`:

In `create_parser()`, after the compare subparser block (line 58), add:
...
In `main()`, before the `else` branch (line 136), add:
...
Add the `_run_explain` function after `_run_analysis`:
...
Run: `uv run pytest -q`
Expected: All 71+ tests pass (existing tests should be unaffected)

**Step 6: Commit**

```bash
git add src/bounce_house/cli.py tests/test_explain.py
git commit -m "feat: wire explain subcommand into CLI with fuzzy matching"
```
...
**Step 1: Run the explain command manually to verify**

Run: `uv run bounce-house explain`
Expected: Overview of all metrics with module headers

Run: `uv run bounce-house explain loudness`
Expected: Detailed loudness module documentation

Run: `uv run bounce-house explain lufs --technical`
Expected: Single metric with technical details

Run: `uv run bounce-house explain cent`
Expected: Resolves to spectral centroid

Run: `uv run bounce-house explain xyzzy`
Expected: Error message with suggestions

**Step 2: Update TODO.md**

Move the metric documentation item from Open to Done:
...
```bash
git add TODO.md
git commit -m "docs: mark metric documentation as done in TODO.md"
```

...
from bounce_house.diagnostics import Diagnosis, evaluate_diagnostics, resolve_metric
...
class TestEdgeCases:
    ...
    def _make_results(self, **overrides):
        ...
        return TestEvaluateDiagnostics._make_results(TestEvaluateDiagnostics(), **overrides)
        ...
        for stage in ("master", "mix"):
            ...
            for p in profile.patterns:
                assert required.issubset(p.keys()), (
                    f"Pattern {p.get('pattern')} in {stage} missing keys"
                )
